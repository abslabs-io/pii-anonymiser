...
**Observed:** Ran Qwen3-1.7B with prompt v2 and fixture v1 in headless Chromium 148 using SwiftShader. Because the adapter lacks `shader-f16`, the app selected `Qwen3-1.7B-q4f32_1-MLC`. The model downloaded and loaded successfully. Its first contact-details fixture exceeded the 120-second generation timeout without returning content, so no detection-quality score was possible. Later attempts reused the persistent cache. The [reviewed evidence](evidence/runtime-limitations/2026-09-11-qwen3-1.7b-software-gpu.json) records the environment and limitation.
...
**Initial evidence status:** No real Gemini Nano inference ran in the development environment. Unit and simulated browser tests exercised API availability, structured requests, clean-session isolation, download progress, cancellation, timeouts, strict parsing, routing, export metadata, and the shared workbench/evaluation flow. These remain infrastructure checks.
...
### First reviewed Gemini Nano run

**Observed:** The author supplied a complete five-case export from Chrome 152 on Windows. Chrome reported the runtime as `gemini-nano@chrome-managed`; the exact model variant, GPU, numeric sampling settings, finish reasons, and output-token counts are unavailable through the Prompt API. All five responses completed, passed the JSON Schema and exact-span validation, and produced anonymised output. Total inference time was 9.362 seconds, with a 1.872-second mean and 1.582-second median.

The run found **10/11 expected occurrence spans, missed 1, and added 7 extras**: 58.8% precision, 90.9% recall, and 71.4% F1. This is also the end-to-end output score because no document was rejected. Contact details, account reference, and instruction-in-text were perfect. The address case found the person and birth date but split the expected complete address into three `ADDRESS` tokens; all address characters were redacted, but exact-span scoring counts this as one miss and three extras. The no-PII case incorrectly redacted `Friday` as a birth date and `12`, `3`, and `2,400` as IDs.

**Comparison:** This is the strongest single fixture-v1 result so far. The reviewed WebLLM 1.7B raw detections scored 9/11 correct with 8 extras (64.3% F1), while Gemini Nano scored 10/11 with 7 extras (71.4% F1). Gemini also avoided atomic rejection, so its end-to-end F1 remained 71.4% rather than WebLLM 1.7B's 52.2%. Timing is not controlled: Chrome does not expose the Gemini variant or execution device, and the reports do not identify the desktop GPU. One run cannot establish stability because Chrome's default sampling controls are unavailable. See the [reviewed evidence summary](evidence/fixture-v1/2026-09-15-gemini-nano-prompt-v2.json).

### Dependency audit after the Transformers.js 1.7B load failure

**Observed:** The author reported that another attempt to load the Transformers.js 1.7B experiment did not work. No model response ran, so this remains runtime/deployability evidence rather than detection-quality evidence.

**Dependency audit:** The npm registry identifies `@huggingface/transformers` 4.2.0 as the latest release, matching the installed version. It pins `onnxruntime-web` `1.26.0-dev.20260416-b7804b056c`; the standalone ONNX Runtime Web latest release is 1.30.0. An unsupported package override would no longer test the published Transformers.js stack, so none was added. WebLLM remains intentionally pinned at 0.2.82 rather than latest 0.2.85 because the later GPU-cache regression is already recorded above. The model-loading failure is not explained by stale application dependencies.

**Upstream evidence:** ONNX Runtime issue [#29455](https://github.com/microsoft/onnxruntime/issues/29455) measured a transient full-model JS-side copy during WebGPU session creation through Transformers.js 4.2.0 and ONNX Runtime Web. The issue was closed by a merged change, but that does not establish that the fix is present in the ONNX development build pinned by Transformers.js 4.2.0. Open Transformers.js issues [#1518](https://github.com/huggingface/transformers.js/issues/1518) and [#1653](https://github.com/huggingface/transformers.js/issues/1653) record the same `ERROR_CODE: 6, ERROR_MESSAGE: std::bad_alloc` during WebGPU session creation for other models and browsers. This supports a runtime-stack memory limitation as the leading explanation, but it does not prove the precise cause on the author's machine without memory measurements.

**Decision:** Keep the failed 1.7B load as a meaningful limitation of the current published Transformers.js path. Do not override its transitive ONNX runtime or repeatedly retry unchanged conditions. Revisit the 1.7B model when a Transformers.js release incorporates a newer ONNX Runtime Web build or documents a supported lower-peak loading path.

## 2026-09-15 — Fixture set v2 for technology comparisons

**User direction:** Keep the experiment focused on comparing browser-local technologies and expand the evaluation only if it materially improves that comparison.

**Decision:** Expand the diagnostic suite from five to fifteen invented cases and identify new exports as fixture set v2. Preserve every reviewed v1 result unchanged; results from different fixture-set versions must not be compared as if they used the same inputs. Fifteen cases keep desktop runs practical while reducing the chance that one clean-negative example or one address-boundary convention determines the apparent ranking.

The ten additions isolate capabilities and failure modes relevant to the technology comparison: Unicode names, an international phone format, `ID` versus `ACCOUNT`, a birth date beside an ordinary date, a multiline address, repeated account occurrences, collision with token-like source text, a second clean negative containing ordinary codes and quantities, name components that are ordinary words, and a non-Latin name. All values are invented. Prompt v2, model settings, parsing, exact-span policy, atomic rejection, and deterministic replacement remain unchanged.

This is still a diagnostic suite, not a representative PII benchmark or a safety claim. Its value is comparative: it supplies more observations for structured-generation reliability, false positives, exact-span behavior, Unicode handling, latency, and runtime completion while keeping the input constant across technologies. Model-family comparisons should use v2 from this point onward; the existing v1 evidence remains the historical baseline.

**Verification:** `npm run format` and `npm run check` passed, including the production build and all 77 unit/lifecycle tests. All 19 Playwright workflows passed with simulated detectors after their fixture-count assertions were updated. These checks establish fixture orchestration and application behavior, not model quality on fixture set v2.

### First automated fixture-v2 runtime comparison

**Environment and method:** The WSL development environment served the application locally while Windows Node ran the existing Playwright evaluator against headless Windows Chrome 152.0.7977.83. Each runtime used a separate dedicated Windows Chrome profile and cache; the author's normal browser profile was not accessed. GPU details were not captured, but both runtime IDs confirm `q4f16` selection. The runs used fixture set v2, prompt v2, and the existing runtime-specific parsers and generation constraints.

The first headed WebLLM attempt loaded but timed out on case one and produced an incomplete report. A cached headless retry completed all inference calls, but Windows Chrome closed during the asynchronous download handoff before Playwright copied the report into WSL. The runner now captures the JSON synchronously in the same page action that creates the export. A subsequent run completed and preserved the report. These failed attempts are infrastructure observations, not additional model-quality samples.

**WebLLM 0.6B observed:** `Qwen3-0.6B-q4f16_1-MLC` completed all fifteen generations. Thirteen documents produced output; one response failed exact-span validation and one failed conflicting-category validation. Across output actually published by the app, it produced **17/30 correct expected occurrences, 13 misses, and 28 extras**: 37.8% precision, 56.7% recall, and 45.3% F1. Reviewing the two rejected raw responses raised diagnostic model intent to 19/30 correct with 38 extras (43.7% F1), but rejected detections never became output. Total generation time was 165.514 seconds. See the [reviewed WebLLM v2 evidence](evidence/fixture-v2/2026-09-15-qwen3-0.6b-webllm-fixture-v2.json).

**Transformers.js 0.6B observed:** `onnx-community/Qwen3-0.6B-ONNX#q4f16` completed all fifteen generations. Twelve documents produced output; two responses failed entity validation and one failed exact-span validation. Published output contained **5/30 correct expected occurrences, 25 misses, and 30 extras**: 14.3% precision, 16.7% recall, and 15.4% F1. Reviewing the rejected responses raised diagnostic model intent to 10/30 correct with 44 extras (23.8% F1). Total generation time was 102.941 seconds. See the [reviewed Transformers.js v2 evidence](evidence/fixture-v2/2026-09-15-qwen3-0.6b-transformersjs-fixture-v2.json).

**Comparison:** Transformers.js was faster in these same-browser, same-machine runs, but its detection and end-to-end output were substantially worse. This compares the complete technology paths, including different model export formats and structured-generation capabilities; it does not isolate inference-engine speed. Both models over-redacted both clean-negative fixtures, confirming that the earlier false positives were not peculiar to one v1 passage.

**Runner correction:** The console summary previously totalled only app-scored documents, which made the denominator omit expected spans from rejected documents. It now derives the suite-wide expected occurrence count from the fixtures and reports rejected-document expectations as end-to-end misses while retaining the app-scored subtotal. The reviewed evidence above applies that corrected definition to the captured reports.

**Gemini Nano status:** Windows Chrome launched successfully from WSL automation, but the dedicated clean profile reported the Chrome-managed model as unavailable, even after the runner allowed background networking and component updates. The normal browser profile was deliberately not accessed or copied. Gemini Nano fixture-v2 evidence therefore still requires a manual run in the author's eligible Chrome profile.

### WebLLM 1.7B fixture-v2 run

**Observed:** At the author's request, the same WSL-to-Windows automation ran `Qwen3-1.7B-q4f16_1-MLC` through WebLLM 0.2.82 in headless Windows Chrome 152.0.7977.83. All fifteen generations completed without a timeout or runtime failure. Thirteen documents produced output; contact details failed exact-span validation and the ambiguous-name case failed conflicting-category validation.

Across output actually published by the app, 1.7B produced **19/30 correct expected occurrences, 11 misses, and 23 extras**: 45.2% precision, 63.3% recall, and 52.8% F1. Reviewing the two rejected responses raised diagnostic raw model intent to 23/30 correct with 31 extras (54.8% F1). The contact-details response exactly matched the previously reviewed v1 failure: it found both name occurrences and the phone but missed the real email and invented two non-source literals. Total generation time was 277.638 seconds, with an 18.509-second mean and 18.075-second median. See the [reviewed WebLLM 1.7B v2 evidence](evidence/fixture-v2/2026-09-15-qwen3-1.7b-webllm-fixture-v2.json).

**Comparison with WebLLM 0.6B:** Under the same fixture set, prompt, WebLLM version, browser version, machine, headless mode, and q4f16 export family, 1.7B increased end-to-end correct occurrences from 17 to 19, reduced misses from 13 to 11, and reduced extras from 28 to 23. End-to-end F1 rose from 45.3% to 52.8%. Total generation time increased from 165.514 to 277.638 seconds, a factor of 1.68. The larger model is directionally better, but the improvement is modest and neither size is reliable enough for unattended anonymisation.

### Gemini Nano fixture-v2 run

**Observed:** The author ran fixture set v2 manually in their eligible normal Chrome 152 profile and supplied the complete export. All fifteen Prompt API calls completed. Fourteen documents produced output; the ambiguous-name response assigned `May` to both `PERSON` and `DATE_OF_BIRTH`, so conflicting-category validation rejected that document.

Across output actually published by the app, Gemini Nano produced **26/30 correct expected occurrences, 4 misses, and 17 extras**: 60.5% precision, 86.7% recall, and 71.2% F1. Reviewing the rejected response raised diagnostic raw model intent to 27/30 correct with 21 extras (69.2% F1). Total generation time was 26.512 seconds, with a 1.767-second mean and 1.675-second median. Chrome does not expose the exact model variant, execution device, numeric sampling controls, or output-token count. See the [reviewed Gemini Nano v2 evidence](evidence/fixture-v2/2026-09-15-gemini-nano-fixture-v2.json).

Six cases were perfect: contact details, Unicode contact details, international phone format, ID/account distinction, repeated account reference, and existing token-like text. Both clean-negative cases still failed: the original negative produced seven extra replacements and the added negative produced two. The model also split complete addresses, returned only the local part of one email, labelled an ordinary review date as a birth date, and labelled an order reference as an ID.

**Technology-path comparison:** Gemini Nano had the strongest end-to-end v2 result, ahead of WebLLM 1.7B at 52.8% F1, WebLLM 0.6B at 45.3%, and Transformers.js 0.6B at 15.4%. Its 26.512-second total was also substantially lower than 277.638, 165.514, and 102.941 seconds respectively. This is a same-machine, same-browser-version comparison of complete paths, not a controlled model-only benchmark: Gemini ran in the author's normal headed profile, the Qwen models ran in dedicated headless profiles, and Chrome manages Gemini's hidden model and sampling. One Gemini run does not establish output stability.

### Evidence directory organization

**Decision:** Group reviewed summaries by `fixture-v1`, `fixture-v2`, and `runtime-limitations`, with an index describing what can be compared. The v1 files remain historical evidence for prompt, parser, and model-size decisions, so they are not labelled `old` or archived as obsolete. The software-GPU timeouts are separated because they document environment limitations rather than detection quality. This organization changes paths only; evidence content and conclusions are unchanged.
