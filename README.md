...
4. Use **Evaluation** to run 15 labelled examples sequentially and export a JSON report.
...
Evaluation checks exact span and category matches, including repeated occurrences. It reports correct, missed, and extra detections. Completed validation failures remain failures and produce no partial output, but do not prevent later fixtures from running. Runtime failures stop the run. The 15 invented cases are a diagnostic comparison set, not an accuracy benchmark. Existing reviewed results use the original five-case fixture set v1; new exports identify the expanded set as v2. See the [experiment journal](docs/experiment-journal.md) for actual validation status and findings.

**Current real-model status:** WebLLM has reviewed Windows desktop results. Qwen3-0.6B with prompt v2 scored 8/11 correct spans with 3 missed and 16 extra. See the [reviewed WebLLM 0.6B result](docs/evidence/fixture-v1/2026-09-11-qwen3-0.6b-prompt-v2.json). After the WebLLM rollback, Qwen3-1.7B completed all five inference calls. Its reviewed raw detections scored 9/11 correct with 2 missed and 8 extra, but exact-span validation rejected one complete document. The anonymised output actually published across the fixtures contained only 6/11 correct replacements, with 5 missed and 6 extra. See the [reviewed WebLLM 1.7B result](docs/evidence/fixture-v1/2026-09-11-qwen3-1.7b-prompt-v2-webllm-0.2.82.json). The larger model performed better on this small diagnostic set but remains unsuitable for unattended anonymisation. Software-WebGPU runs are recorded separately for [0.6B](docs/evidence/runtime-limitations/2026-09-10-software-gpu.json) and [1.7B](docs/evidence/runtime-limitations/2026-09-11-qwen3-1.7b-software-gpu.json); both timed out and are not representative of desktop GPU performance.

The first Transformers.js hardware-WebGPU run used Qwen3-0.6B q4f16 in Firefox 154. All five generations completed, but every response was Markdown-fenced JSON, so parser v1 rejected every document and published no anonymised output. A diagnostic review inside the fences found 6/11 expected spans, 5 misses, and 14 extras from 20 returned occurrence spans (30.0% precision, 54.5% recall, 38.7% F1). See the [reviewed Transformers.js 0.6B result](docs/evidence/fixture-v1/2026-09-15-qwen3-0.6b-transformersjs-4.2.0.json). Browser and runtime differences mean its 66.590-second total should not be treated as a controlled timing comparison with the earlier WebLLM run. Parser v2 subsequently added the exact fence normalization described above; the v1 result remains the preserved formatting baseline.

The parser-v2 0.6B rerun returned exactly the same five raw responses and token counts. Fence normalization allowed three documents to produce output; one response still failed because it used an unsupported category, and one failed because it contained a non-source literal. Across output actually published by the app, only 2/11 expected spans were replaced, 9 were missed, and 7 incorrect replacements were added (22.2% precision, 18.2% recall, 20.0% F1). See the [reviewed parser-v2 result](docs/evidence/fixture-v1/2026-09-15-qwen3-0.6b-transformersjs-parser-v2.json). The parser fix resolved the formatting problem but did not make the 0.6B model reliable.

The first reviewed Gemini Nano run completed all five fixtures and produced output for every document. It found 10/11 expected occurrences with 1 miss and 7 extras (58.8% precision, 90.9% recall, 71.4% F1). Four extras came from redacting ordinary values in the no-PII fixture, so this strongest result in fixture set v1 is still unsuitable for unattended anonymisation. Chrome controls the model variant and sampling; repeat runs are needed to measure stability. See the [reviewed Gemini Nano result](docs/evidence/fixture-v1/2026-09-15-gemini-nano-prompt-v2.json).

The fixture-v2 comparison used Windows Chrome 152 on the same machine. Gemini Nano produced the strongest published output: 26/30 correct expected occurrences with 4 misses and 17 extras (71.2% F1), with one document rejected. WebLLM 0.6B published 17/30 correct with 13 misses and 28 extras (45.3% F1). WebLLM 1.7B improved to 19/30 correct with 11 misses and 23 extras (52.8% F1), but took 1.68 times as long; each WebLLM run rejected two documents. Transformers.js 0.6B published 5/30 correct with 25 misses and 30 extras (15.4% F1), with three documents rejected. See the reviewed [Gemini Nano v2](docs/evidence/fixture-v2/2026-09-15-gemini-nano-fixture-v2.json), [WebLLM 0.6B v2](docs/evidence/fixture-v2/2026-09-15-qwen3-0.6b-webllm-fixture-v2.json), [WebLLM 1.7B v2](docs/evidence/fixture-v2/2026-09-15-qwen3-1.7b-webllm-fixture-v2.json), and [Transformers.js v2](docs/evidence/fixture-v2/2026-09-15-qwen3-0.6b-transformersjs-fixture-v2.json) results. Gemini was much faster in this complete-path comparison, but Chrome does not expose its exact model variant, execution device, or sampling controls. All paths remained unsuitable for unattended anonymisation. The [evidence index](docs/evidence/README.md) separates current comparisons, historical baselines, and runtime limitations.
